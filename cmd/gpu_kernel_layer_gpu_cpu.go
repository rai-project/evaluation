...
var gpuKernelLayerGPUCPUCmd = &cobra.Command{
	Use:     "layer_gpu_cpu",
	Aliases: []string{},
	Short:   "Get GPU vs CPU duration within each layer from system library traces in a database. Specify model name as `all` to list information of all the models.",
		...
		run := func() error {
			...
			if sortOutput {
				sort.Slice(summary, func(ii, jj int) bool {
					return summary[ii].GPUDuration > summary[jj].GPUDuration
				})
			}
